...
```typescript
const programKeywords = programType === "BJJ"
  ? ["bjj classes", "brazilian jiu-jitsu", location.locationName, "Knox", "Boronia", "martial arts"]
  : ["karate classes", location.locationName, "Knox", "Boronia", "martial arts"];
```
...
**Fix Needed:**
```astro
alt={`${program.title} classes at Kando Martial Arts Knox in ${location.locationName}`}
alt={`${classItem.heading} training at Kando Martial Arts Knox dojo in ${location.locationName}`}
```
...
<!-- ... other sections ... -->
<Testimonials /> <!-- Duplicate without props -->
```
...
**Remove:**
```astro
<Testimonials /> <!-- This duplicate line -->
```
